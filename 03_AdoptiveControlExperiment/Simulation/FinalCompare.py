...
env_params = [
    ...
    [600.0, 0.29, 9.1819, 1.1606],  # Miso Soup
...
fig_height_inch = 8.0 / 2.54 # 下部のラベルと凡例のスペース確保のために少し高くしました
...
# ==========================================
# 5. 実機実験の数値を直書きする処理（💥ここで自由に位置を調整できます！）
# ==========================================
# [Pure Waterのズレ, Miso Soupのズレ, Miso+Tofuのズレ] の順です。
# 例：(0, 15) は「真上」にズラす。(20, -10) は「右下」にズラす。(-25, 0) は「真左」にズラす。
offset_ol  = [(0, 30), (0, 25), (0, 10)]      
offset_pid = [(0, 15), (0,15), (0, 10)]     # Pure WaterとMisoの重なりを避ける
offset_str = [(0, 35), (0, -15), (0, -15)]
...
# 6. 装飾とレイアウト調整
# ==========================================
ax.set_ylabel('Steady-State Absolute Error [%]')
ax.set_xticks(x)
...
ax.annotate('', xy=(1.0, -0.25), xycoords='axes fraction', xytext=(0.0, -0.25),
            arrowprops=dict(arrowstyle="->", color="black", lw=1.2))
ax.text(0.5, -0.33, 'Increasing Soup Complexity', transform=ax.transAxes, 
...
legend_ctrl = [
    ...
    Line2D([0], [0], color=color_pid, marker='s', linestyle='-', lw=lw_exp, markersize=ms, label='PID'),
    Line2D([0], [0], color=color_str, marker='D', linestyle='-', lw=lw_exp, markersize=ms+1, label='Adaptive')
]

# 💡 1段目：コントロール手法
leg1 = ax.legend(handles=legend_ctrl, loc='upper center', bbox_to_anchor=(0.5, -0.42), 
                 ncol=3, frameon=False, fontsize=8, columnspacing=0.8, handletextpad=0.4)

# 💡 2段目：カスタマイズした「Experiment」と「Simulation」の特殊アイコン
leg2 = ax.legend(handles=[ExpDummy(), SimDummy()], labels=['Experiment', 'Simulation'],
                 ...
                 handlelength=3.5, # 💡 マーカーを3つ並べるためにアイコンの横幅を長めに設定！
                 handler_map={ExpDummy: ExpHandler(), SimDummy: SimHandler()})

ax.add_artist(leg1) 

# 💡 全体の下部スペースの比率を大きくして、切れないようにします
