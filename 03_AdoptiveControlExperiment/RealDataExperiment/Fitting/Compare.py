...
def process_and_fit(data_text, m_water):
    df = pd.read_csv(io.StringIO(data_text.strip()), sep='\t')
    df = df.dropna(subset=['C (g)', 'T (°C)', 'σ (mS/cm)'])
    
    # X軸: 塩分濃度 C(%) の計算
    x_data = (df['C (g)'] / m_water) * 100
    
    # Y軸: 温度補正済み導電率 σ_comp の計算
    y_data = df['σ (mS/cm)'] / (1 + a_temp * (df['T (°C)'] - T_room))
    
    # 線形回帰で a (傾き), b (切片), r_value (相関係数) を計算
    a, b, r_value, _, _ = linregress(x_data, y_data)
    r_squared = r_value**2
    
    print(f"--- フィッティング結果 (Water: {m_water}g) ---")
    print(f"a (感度) = {a:.4f}, b (ベースライン) = {b:.4f}, R^2 = {r_squared:.4f}")
    
    return x_data.values, y_data.values, a, b, r_squared

# 3つの環境のデータを処理
x1, y1, a1, b1, r2_1 = process_and_fit(data_env1, m_water=600.0)
x2, y2, a2, b2, r2_2 = process_and_fit(data_env2, m_water=580.0)
x3, y3, a3, b3, r2_3 = process_and_fit(data_env3, m_water=480.0)
...
# Env 1 (純水): 黒 / 丸 / 実線
# ※ labelパラメータは外し、戻り値を変数(scatter1, line1)として受け取ります
...
# --- 凡例の微調整 (ここがポイント) ---
# 点と線をペアにしてhandlesに渡し、1つのラベルに対応させます
...
ax.legend(
    ...
    edgecolor='black', 
    fancybox=False, 
    handlelength=2.5,     # 点と線の両方を表示するため、描画スペースを少し長めに確保します
    fontsize=7.5,         
    labelspacing=0.35     
)
